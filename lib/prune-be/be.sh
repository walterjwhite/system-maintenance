...
_prune_be_by_age() {
	# destroy old BE
	# BE must not be active (N) and not scheduled to be active for next reboot (R) AND at least $_BE_RETENTION_PERIOD old
	local be_dtl=""
...
_prune_be_details() {
	_BE_NAME=$(echo "$be_dtl" | awk '{print$1}')
	_BE_CR_DT=$(echo "$be_dtl" | awk '{print$5" "$6}')
