...
_apply_patch() {
	...
	if [ -z "$_JAIL_PATH" ]; then
		# the current and future BE != system be
		if [ "$_ACTIVE_BE" != "$_SYSTEM_BE_WITH_SEQUENCE" ]; then
			_error "reboot into latest BE: $_ACTIVE_BE ($_SYSTEM_BE_WITH_SEQUENCE) before applying patches"
		fi
	else
		_warn "no need to reboot, patching jail: $_JAIL_PATH"
	fi

	_info "applying patches, BE: $_SYSTEM_BE_WITH_SEQUENCE"

	_PATCH_FNCTN=$(head -1 $_LAST_SEQUENCE_FILE)
	_patch_$_PATCH_FNCTN

	# record the date the system was patched
	if [ -n "$_DRY_RUN" ]; then
