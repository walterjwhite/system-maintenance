_be() {
	...
	_SYSTEM_BRANCH=$(head -1 $_SYSTEM_CONFIGURATION_FILE)
	...
	if [ -z "$_JAIL_NAME" ]; then
		# N=now, R=reboot
		# this means that the current BE does != the one that will be active after rebooting
	...
	if [ -n "$_LAST_SEQUENCE_FILE" ]; then
		...
		_FIRST_BOOT=1
		# create a new BE on first booting the system
